...
"""Importiamo il modulo preprocessing.py che contiene i dati già puliti e normalizzati. Dividiamo in:
...
import Data.STEP_028.Preparazione_dataset as prep
...
def build_model(input_dim, output_dim, lr=1e-2):
    model = models.Sequential([
        layers.Input(shape=(input_dim,)),             #layer di input con dimensione dei dati di addestramento
        layers.Dense(128, activation='relu'),   #2° (hydden) layer con 128 neuroni e funzione di attivazione ReLU
        layers.Dropout(0.2),                          #dropout del 20% (spegnimento del 20% dei neuroni)per regolarizzazione
        layers.Dense(128, activation='relu'),    #3° (hydden) layer con 64 neuroni e funzione di attivazione ReLU
