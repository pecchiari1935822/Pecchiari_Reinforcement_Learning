...
class SmithDiagram_Reaction_Assiale:
    ...
    '''def get_eta_from_phi(self, phi, deflection):
        """
        Dato phi e deflessione, restituisce eta.
        """
        if deflection not in self.interpolators:
            deflections = list(self.interpolators.keys())
            raise ValueError(f"Deflessione {deflection}° non trovata. "
                             f"Disponibili: {sorted(deflections)}")

        return float(self.interpolators[deflection](phi))

    def get_phi_from_eta(self, eta, deflection):
        """
        Dato eta e deflessione, restituisce phi (inverso).
        """
        ...
        inv_interp = interp1d(
            eta_vals, phi_vals, kind='cubic', bounds_error=False,
            fill_value='extrapolate'
        )

        return float(inv_interp(eta))

    def get_nearest_efficiency(self, phi, eta):
        """
        Trova l'efficienza più vicina ai valori phi, eta dati.
        """
        min_distance = np.inf
        nearest_efficiency = None

        for eff_val, points in self.efficiency_curves.items():
            distances = np.sqrt((points[:, 0] - phi) ** 2 + (points[:, 1] - eta) ** 2)
            min_dist = np.min(distances)

            if min_dist < min_distance:
                min_distance = min_dist
                nearest_efficiency = eff_val

        return nearest_efficiency'''
    ...
    # (Tutta la prima parte della tua classe SmithDiagram rimane invariata fino a "def plot")
        ...
        for efficiency, points in sorted(self.efficiency_curves.items()):
            ...
            if len(valid_points) > 0:
                ax.text(valid_points[-1, 0], valid_points[-1, 1], f'  η={efficiency:.2f}',
                        fontsize=15, verticalalignment='center_baseline', horizontalalignment='center')
