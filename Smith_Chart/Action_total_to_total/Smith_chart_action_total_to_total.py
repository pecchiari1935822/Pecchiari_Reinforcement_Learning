...
class SmithDiagram_Action_total_to_total:
    ...
    def __init__(self, csv_folder):
        ...
        # AGGIUNTA: per evitare extrapolate "fantasy" e per stimare deflessione da (phi,psi)
    ...
    def _parse_efficiency_from_filename(self, csv_file: Path) -> float:
        """
        Supporta: eta_0.92.csv, eta_92.csv (in quest'ultimo caso 92 rimane 92)
        """
    ...
    def _load_csv_files(self):
        ...
        for csv_file in self.csv_folder.glob("defl_*.csv"):
            try:
                # MODIFICA: parse robusto dell'angolo
    ...
    def plot(self, figsize=(12, 9), save_path=None, target_point=None, highlight_deflection=None):
        ...
        for deflection, points in sorted(self.deflection_curves.items()):
            ...
            # MODIFICA: usa davvero lo stile scelto sopra + label per legenda
            ax.plot(
...
fig, ax = smith_action_total_to_total.plot(
    figsize=(12, 9),
    save_path="smith_diagram_action_with_dataset.png"
)

# 6. Scatter dataset sul diagramma
...
for i in range(len(df)):
    ...
    # Deflessione Smith (intero più vicino)
    d_smith = smith_action_total_to_total.estimate_deflection_nearest_integer(
...
# smith_action_total_to_total.plot(target_point=(0.6243, 1.2198), highlight_deflection=100,
